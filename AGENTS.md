...
This is the **milestone MVP** repository: **schema management (M1)** and **read-only schema viewer (M0)** only. Do not add vision-only routes (workflows, roles, BOS explorations, form-preview exploration nav, etc.) here — use [**x99-vision**](https://github.com/dil-wrahn/x99-vision) or [**x99-lab**](https://github.com/dil-wrahn/x99-lab). Read [`PROTOTYPE_SCOPE.md`](./PROTOTYPE_SCOPE.md) and [`PROTOTYPES.md`](./PROTOTYPES.md). Do not assume other prototype repos are open in the workspace.
...
1. **Canonical requirements:** [Confluence — Custom attributes prototype docs](https://diligentbrands.atlassian.net/wiki/spaces/RCP/folder/6953959468) (update specs here first).
2. **Working copies in-repo:** `src/docs/` (e.g. `custom-attributes-product-context.md`, `custom-attributes-milestone-requirements.md`) — use when Confluence is unavailable or for grep context; may lag Confluence.
